...
public class SignatureFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        signatureMap.put(appKey, headerMap.get(appKey));
        signatureMap.put(appSecret, headerMap.get(appSecret));
        signatureMap.put(timestamp, headerMap.get(timestamp));
        signatureMap.put(version, headerMap.get(version));
        signatureMap.put(randomStr, headerMap.get(randomStr));
        // 加入URL参数
        ...
        if (!signatureStr.equals(Pines.getOrException(headerMap, signatureKey))) {
            throw new IllegalArgException(CodeInfoEnum.SIGNATURE_ERROR);
        }
